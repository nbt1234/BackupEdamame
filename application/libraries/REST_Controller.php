...
abstract class REST_Controller extends \CI_Controller
{
    ...
    public function _remap($object_called, $arguments = [])
    {
        ...
        if ($this->config->item('rest_enable_keys') && $use_key && $this->_allow === false) {
            ...
            $this->response([
                $this->config->item('rest_status_field_name') => 'fail',
    ...
    protected function _detect_api_key()
    {
        ...
        $api_key_user = $this->config->item('rest_keys_user');

        // Work out the name of the SERVER entry based on config
        $key_name = 'HTTP_' . strtoupper(str_replace('-', '_', $api_key_variable));

        $key_user = 'HTTP_' . strtoupper(str_replace('-', '_', $api_key_user));
        ...
        $user_key = isset($this->_args[$api_key_user]) ? $this->_args[$api_key_user] : $this->input->server($key_user);


        // Find the key from server or arguments
        if (isset($key)) {
            if (!($row = $this->rest->db->where([$this->config->item('rest_key_column')=> $key,$this->config->item('rest_keys_user')=>$user_key])->get($this->config->item('rest_keys_table'))->row())) {
                return false;
            }
